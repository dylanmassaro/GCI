...
class SourcesPage(Screen):
    def __init__(self, **kwargs):
        ...
        layout.add_widget(Button(
            **BUTTON_STYLE,
            text="Lifestyle and the Prevention of Type 2 Diabetes: A Status Report",
        ...
        layout.add_widget(Button(
            **BUTTON_STYLE,
            text="Type 2 Diabetes: What It Is, Causes, Symptoms, & Treatment",
            background_color=COLORS['teal_3'],
...
class InputPage(Screen):
    ...
    def finish_submission(self, selections):
        score_text, link = self.calculate_fake_risk(selections)
        results_page = self.manager.get_screen('results')
        results_page.update_result(score_text, link)
    ...
    def calculate_fake_risk(self, selections):
        ...
        return f"{total} ({risk_level})", link
